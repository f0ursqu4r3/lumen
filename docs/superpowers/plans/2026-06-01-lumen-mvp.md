...
<template>
  <section class="space-y-4">
    ...
    >
      ...
      <li v-if="!projects?.length" class="px-3 py-2 text-sm text-neutral-500">
        No projects.
      </li>
    </ul>
  </section>
...
<template>
  <span
    ...
    :class="
      open ? 'bg-green-100 text-green-800' : 'bg-neutral-200 text-neutral-700'
    "
...
<template>
  <span
    class="inline-flex items-center gap-1 text-xs text-neutral-600"
    :title="username"
  >
    <img
      v-if="avatarUrl"
...
it("adds a note when the comment form is submitted", async () => {
  ...
  vi.spyOn(mod, "useAddNote").mockReturnValue({
    mutate,
    isPending: ref(false),
  } as never);
...
it("creates an issue from the new-issue form", async () => {
  const mutate = vi.fn();
  const mod = await import("@/composables/useIssueMutations");
  vi.spyOn(mod, "useCreateIssue").mockReturnValue({
    mutate,
    isPending: ref(false),
  } as never);
