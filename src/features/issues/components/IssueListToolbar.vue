...
<template>
  <!-- The two control rows are one cluster: a tight internal rhythm, with the
       larger gap down to the active filters and content owned by the parent. -->
  <div class="space-y-2.5">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <button
        ...
        class="grid size-9 shrink-0 place-items-center rounded-lg text-muted-foreground transition-colors duration-150 outline-none hover:bg-accent hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/60 active:scale-[0.97]"
    ...
    <!-- Toolbar row 2: ordering controls on the left, filtered count on the right.
       The left controls differ by view (list groups vs board columns); the count
       trails both so the result tally always sits in the same spot. -->
    <div class="flex flex-wrap items-center gap-2">
      ...
      <Select v-if="view === 'list'" v-model="groupKey">
      ...
      <Select v-else v-model="boardScope">
        <SelectTrigger class="h-8 w-44 text-xs" aria-label="Column grouping">
      ...
      <button
        v-if="hasCustomOrder"
        type="button"
        :data-testid="view === 'list' ? 'reset-group-order' : 'reset-column-order'"
        class="inline-flex h-8 items-center gap-1.5 rounded-lg px-2 text-xs font-medium text-muted-foreground transition-colors duration-150 outline-none hover:bg-accent hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/60 active:scale-[0.97]"
      ...
      <!-- Filtered result tally — quiet mono, mirrors the Project Picker count. -->
