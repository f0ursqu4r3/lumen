...
describe('IssueComposer', () => {
  it('disables Create until the title is non-empty', async () => {
    mountComposer()
    await flushPromises()
    expect(q('[data-testid="composer-submit"]')!.hasAttribute('disabled')).toBe(true)
    await setField('[data-testid="composer-title"]', 'Fix it')
    expect(q('[data-testid="composer-submit"]')!.hasAttribute('disabled')).toBe(false)
  })
  ...
  it('submits title, description, labels and assigneeIds', async () => {
    ...
    await click('[data-testid="label-option-bug"]')
