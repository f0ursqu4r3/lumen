...
describe('Scratchpad', () => {
  ...
  it('renders a previously saved value into the textarea', () => {
    localStorage.setItem('lumen:scratchpad:grp/proj#9', JSON.stringify('saved note'))
  ...
  it('persists typing to localStorage', async () => {
    ...
    await nextTick()
    expect(localStorage.getItem('lumen:scratchpad:grp/proj#9')).toBe(JSON.stringify('typed note'))
  })
  ...
  it('is collapsed by default (textarea hidden)', () => {
    const w = mount(Scratchpad, { props: { fullPath: 'grp/proj', iid: '9' } })
    expect(w.get('[data-testid="scratchpad-toggle"]').attributes('aria-expanded')).toBe('false')
  ...
  it('expands when the header toggle is clicked', async () => {
    ...
    expect(w.get('[data-testid="scratchpad-toggle"]').attributes('aria-expanded')).toBe('true')
    expect(w.get('textarea').attributes('style') ?? '').not.toContain('display: none')
  })
  ...
  it('shows a content marker only when the note has content', () => {
    ...
    localStorage.setItem('lumen:scratchpad:grp/proj#8', JSON.stringify('has content'))
    const withContent = mount(Scratchpad, {
      props: { fullPath: 'grp/proj', iid: '8' },
    })
    expect(withContent.find('[data-testid="scratchpad-marker"]').exists()).toBe(true)
  })

  it('persists the open state per issue', async () => {
    const w = mount(Scratchpad, { props: { fullPath: 'grp/proj', iid: '9' } })
    await w.get('[data-testid="scratchpad-toggle"]').trigger('click')
    expect(localStorage.getItem('lumen:scratchpad-open:grp/proj#9')).toBe(JSON.stringify(true))
    const w2 = mount(Scratchpad, { props: { fullPath: 'grp/proj', iid: '9' } })
    expect(w2.get('[data-testid="scratchpad-toggle"]').attributes('aria-expanded')).toBe('true')
  })
})
