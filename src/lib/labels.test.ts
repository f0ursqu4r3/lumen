...
describe('semantic scopes', () => {
  ...
  it('maps the priority label set to its requested icons', () => {
    expect(
      ['Critical', 'FastTrack', 'High', 'Medium', 'Low'].map(
        (level) => priorityOf([{ title: `priority::${level}`, color: '#fff' }])?.icon,
      ),
    ).toEqual(['AlertOctagon', 'Zap', 'ArrowUpCircle', 'MinusCircle', 'ArrowDownCircle'])
  })
