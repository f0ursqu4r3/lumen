...
const PRIORITY: Record<Priority, PriorityMeta> = {
  high: { level: 'high', color: '#ef4444', icon: 'arrow-up', label: 'High priority', weight: 3 },
  medium: { level: 'medium', color: '#f59e0b', icon: 'equal', label: 'Medium priority', weight: 2 },
  low: { level: 'low', color: '#94a3b8', icon: 'minus', label: 'Low priority', weight: 1 },
}
...
const TYPE: Record<string, TypeMeta> = {
  ...
  ENH: { code: 'ENH', icon: 'sparkles', color: '#8b5cf6', label: 'Enhancement' },
  REF: { code: 'REF', icon: 'recycle', color: '#d97706', label: 'Refactor' },
  INT: { code: 'INT', icon: 'plug', color: '#7c3aed', label: 'Integration' },
  TEST: { code: 'TEST', icon: 'flask-conical', color: '#14b8a6', label: 'Test' },
}
...
export function statusOf(labels: readonly LabelLike[]): ParsedLabel | null {
  for (const l of labels) {
    const p = parseLabel(l.title, l.color)
    if (scopeIs(p, 'assigned') || scopeIs(p, 'workflow') || scopeIs(p, 'status'))
      return p
  }
  return null
}
